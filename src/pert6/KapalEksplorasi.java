...
public class KapalEksplorasi extends KendaraanGalaksi {
    ...
    // Jika cukup → kurangi energi can& tampilkan jarak perjalanan.
    @Override
    public void jelajah(int jarak) {
        int konsumsiEnergi = jarak * 2;  // 2% per km

        if (getLevelEnergi() < konsumsiEnergi) {
            System.out.println("Energi tidak mencukupi untuk menjelajah sejauh " + jarak + " km.");
        } else {
            kurangiEnergi(konsumsiEnergi);   // <-- SUDAH BISA KURANGI ENERGI
    ...
    public void isiEnergi(int jumlah) {
        tambahEnergi(jumlah);
        System.out.println("Mengisi energi sebesar " + jumlah + "%. Energi sekarang: " + getLevelEnergi() + "%");
    }
